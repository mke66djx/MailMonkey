...
# -*- coding: utf-8 -*-
"""
Direct Mail Campaign - Print Safe PDF Version (Windows-focused, enhanced)
Author: ChatGPT (for Ed Beluli)

What this does
- Generates personalized PDF letters from a CSV (ReportLab)
- Title-cases owner names + street names; extracts street core from full address
- Adds per-letter footer with batch/date + sequence (i of N)
- Prints via SumatraPDF.exe silently to the chosen printer
- Tracks each print job in Windows spooler until it completes (or errors)
- Polls BOTH job status (Win32_PrintJob) and printer status (PRINTER_INFO_2)
- Logs to print_state.csv and supports --resume to skip already-printed rows
- Options: --no-halt, --list-printers, --batch-name, --sumatra path
...
macOS/Linux
  ...
  - Printing uses 'lp' best-effort (no spool tracking on non-Windows)

Usage (PowerShell):
  python direct_mail_pdf_printsafe.py `
    --csv "PropertyTaxDelinquentList_08_2025.csv" `
    --out "output_letters" `
    --name "Ed Beluli" `
    --phone "916-905-7281" `
...
def find_job_by_document(printer_name: str, doc_basename: str, timeout_s: int = 45):
    require_windows_modules()
    c = wmi.WMI()
...
def wait_for_job_complete(printer_name: str, job, max_wait_s: int = 900) -> Tuple[bool, str]:
...
def main():
    ap = argparse.ArgumentParser(description="Direct Mail PDF generator & print-safe runner")
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if WIN and (wmi is not None and win32print is not None):
            ...
            job = find_job_by_document(args.printer, os.path.basename(pdf_path), timeout_s=60)
            if not job:
            ...
            ok, msg = wait_for_job_complete(args.printer, job, max_wait_s=1200)
            if ok:
            ...
            ok, msg = print_non_windows(pdf_path, args.printer)
            if ok:
                successes += 1; append_state(idx, pdf_path, "printed", msg)
                print(f"[PRINTED] Row {idx}: {msg}")
            else:
                failures += 1; append_state(idx, pdf_path, "fail", msg)
                print(f"[ERROR] Row {idx}: {msg}")
                if not args.no_halt: break
                else: continue
