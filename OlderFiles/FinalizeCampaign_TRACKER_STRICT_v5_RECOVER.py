...
# -*- coding: utf-8 -*-
"""
FinalizeCampaign_TRACKER_STRICT_v5_RECOVER.py

Modes:
1) Normal finalize (like v4): append executed log for one campaign, update tracker, tally ZIPs.
2) Disaster recovery: --rebuild-all (or --reindex-all) scans ALL campaign folders under --root
   and rebuilds MasterPropertyCampaignTracker.csv + Zip5_LetterTally.csv from executed logs.
   - Optionally require a marker file to identify campaign folders (--marker-required).
   - You can set/standardize the marker name with --marker-name (default: CAMPAIGN.TAG).
3) Quality of life:
   - --write-marker: drops the marker file into the current --campaign-dir during normal finalize.
   - --rebuild-templates: same as v4 (rebuild template sequence + unique campaigns from logs).

Mailing ZIP remains the source of truth where available.
"""
...
def rebuild_tracker_from_all(args) -> None:
    ...
    for key, rec in agg.items():
        ...
        first = rec["FirstSentDt"].strftime("%-m/%-d/%Y") if rec["FirstSentDt"] else ""
        last = rec["LastSentDt"].strftime("%-m/%-d/%Y") if rec["LastSentDt"] else ""
        final_rows.append({
