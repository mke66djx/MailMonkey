...
def is_trust_case(row):
    primary_first = _first_nonempty_from_row(row, ["Primary First","PRIMARY FIRST","primary first","primary_first","primary first name","primary_first_name"])
    primary_last  = _first_nonempty_from_row(row, ["Primary Last","PRIMARY LAST","primary last","primary_last","primary last name","primary_last_name"])
...
def personalize_letter(row: Dict[str, str], your_name: str, your_phone: str, your_email: str, template_text: str) -> Tuple[str, str, str, str]:
    ...
    owner_first = to_title_case(greeting_name)
...
# Trust/partnership handling: replace "Dear {OwnerFirstName}," with "Primary Name,"
if is_trust_case(row):
    primary_name = _first_nonempty_from_row(row, ["Primary Name","PRIMARY NAME","primary name","primary_name"])
    adjusted = re.sub(r"^Dear\s*\{OwnerFirstName\},\s*\n+", "{SalutationLine}\n\n", template_text, flags=re.M)
    content = adjusted.format(
        SalutationLine=f"{primary_name},",
        OwnerFirstName=owner_first or owner_full,  # still available if template uses it below
        StreetName=street,
        YourName=your_name,
    ...
    display = to_title_case(clean_entity_tokens(owner_full_raw)) if owner_full_raw else "Owner"
    adjusted = re.sub(r"^Dear\s*\{OwnerFirstName\},\s*\n+", "{SalutationLine}\n\n", template_text, flags=re.M)
    content = adjusted.format(
