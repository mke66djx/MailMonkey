...
# -*- coding: utf-8 -*-
"""
BuildMasterCampaignList_v4_ShortTestDefault.py

Updates in this revision:
- Reads prior history ONLY from MasterCampaignTracker/MasterPropertyCampaignTracker.csv
- Does not create or modify any tracker files (read-only).
- Prior filters use CampaignCount and (if present) CampaignNumbers pipe-list to compute last campaign.
- Backward-compatible: if a legacy LastCampaignNumber column happens to exist, it's used as a fallback.

All other behavior remains the same:
- Emits campaign_master.csv using the header from the first mandatory CSV that loads successfully
- Up to 4 mandatory and 2 optional lists
- Address/Owner normalization + de-dup across all lists
- USPS-friendly selection (strict-150 supported)
- ZIP5 extraction from address or common ZIP columns (robust handling of "95835.0")
- Sorted output (ZIP5 → Address → OwnerName)
- Presort reports (ZIP5, ZIP3) and postage estimate
- --debug counters
"""
...
def read_tracker() -> Dict[Tuple[str,str], Dict[str,str]]:
    """
    Reads MasterPropertyCampaignTracker.csv if present; otherwise returns empty dict.
    Keys are (PropertyAddress, OwnerName) normalized to upper + single spaces.
    """
...
def parse_last_campaign_number(info: Dict[str,str]) -> int:
    """
    Prefer parsing from CampaignNumbers (pipe or comma separated).
    Fallback to LastCampaignNumber if present; else 0.
    """
    # Try CampaignNumbers like "1|2|7" or "1,2,7"
...
def passes_prior_rules(k: Tuple[str,str], tracker: Dict[Tuple[str,str],Dict[str,str]],
    ...
    # Last campaign number (derived from CampaignNumbers when available)
    ...
    if min_gap > 0 and last > 0:
        # require that the last campaign is at least min_gap behind current
...
# ------------------------- Main -------------------------
def main():
    ap = argparse.ArgumentParser(description="Build USPS-optimized campaign master list (emits master CSV in the first mandatory file's schema).")
