...
- A mapping CSV (owner, property address, ref code, file stub, page) at --map-out
  If --map-out is omitted, DEFAULT NAME becomes:
    <csv_basename>_<N>_refs.csv
  where <N> is the number of pages generated from the CSV.
...
def write_single_letter_pdf(out_dir: str, filestub: str, content: str, ref_code: str, csv_base: str, total_rows: int) -> str:
    ...
    def _meta_first_page(canvas, doc):
        canvas.setTitle(f"{csv_base} - {total_rows} rows")
        canvas.setSubject(f"Source CSV: {csv_base}; Rows: {total_rows}")
        footer(canvas, doc)
    doc.build(story, onFirstPage=_meta_first_page, onLaterPages=footer)
    return path

def write_combined_pdf(out_path: str, contents_with_refs: List[Tuple[str, str, str]], csv_base: str, total_rows: int):
...
def main():
    ...
    ap.add_argument("--map-out", default=None, help="(Optional) Output path for the mapping CSV; default uses <csv_basename>_<N>_refs.csv")
    ...
    # Read CSV first to know how many pages we'll generate
    ...
    # CSV base name (without extension) for metadata & default map name
    csv_base = os.path.splitext(os.path.basename(args.csv))[0]
    # Build default combined PDF name if user used a generic name
    default_comb_name = f"{csv_base}_{total}_letters.pdf"
    ...
    #   <csv_basename>_<N>_refs.csv   (placed next to the combined PDF)
    if args.map_out:
        map_out = args.map_out
    else:
        map_out = os.path.join(comb_dir, f"{csv_base}_{total}_refs.csv")
    ...
    for i, row in enumerate(rows, start=1):
        ...
        # write single file
        path = write_single_letter_pdf(args.outdir, filestub, content, ref_code, csv_base, total)
    ...
    # Combined PDF
    write_combined_pdf(args.combine_out, contents, csv_base, total)
