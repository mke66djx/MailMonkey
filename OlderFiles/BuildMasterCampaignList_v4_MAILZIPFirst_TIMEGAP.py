...
# -*- coding: utf-8 -*-
"""
BuildMasterCampaignList_v4_MAILZIPFirst_TIMEGAP.py

Adds time-based prior filters on top of MAILZIP-first v4 builder:
- --min-days-since-last N    (require LastSentDt to be at least N days ago)
- --last-sent-before DATE    (require LastSentDt < DATE; DATE in YYYY-MM-DD or MM/DD/YYYY)
- --missing-last-sent {fail,include}  (default fail) controls whether rows with no LastSentDt
  are allowed to pass the time filter when one is specified.

Everything else unchanged:
- Reads prior history ONLY from MasterCampaignTracker/MasterPropertyCampaignTracker.csv
- Prior logic supports --prior-exact and --prior-max and --min-gap (campaign-number gap)
- USPS optimization and presort reports, postage estimate, mailing ZIP priority
...
def parse_last_sent_date(info: Dict[str,str]) -> Optional[datetime.date]:
    for key in ("LastSentDt","LastSentDtUTC","LastSentDate"):
        d = try_parse_date(info.get(key,""))
        if d:
            return d
    # as a fallback, try FirstSentDt if Last is absent
    return try_parse_date(info.get("FirstSentDt",""))

# ------------------------- Prior filters -------------------------
def passes_prior_rules(
    k: Tuple[str,str],
    tracker: Dict[Tuple[str,str],Dict[str,str]],
    prior_exact: Optional[int],
    prior_max: Optional[int],
    min_gap: int,
    current_campaign_number: int,
    min_days_since_last: Optional[int],
    last_sent_before: Optional[datetime.date],
    missing_last_policy: str
) -> bool:
    info = tracker.get(k)
    if not info:
        # Never mailed
        ...
        if prior_max is not None and 0 > prior_max:
            return False
        # Time-based filters on "never mailed"
        if min_days_since_last is not None or last_sent_before is not None:
            # With no LastSent date, follow policy
    ...
    try:
        ...
        cnt = 0
    last_cn = parse_last_campaign_number(info)

    # campaign-number gap filter
...
# ------------------------- Main -------------------------
def main():
    ap = argparse.ArgumentParser(description="Build USPS-optimized campaign master list (MAILZIP-first) with optional time filters.")
    ...
    ap.add_argument("--min-days-since-last", type=int, default=None, help="Require LastSentDt be at least N days ago")
    ap.add_argument("--last-sent-before", type=str, default=None, help="Require LastSentDt < this date (YYYY-MM-DD or MM/DD/YYYY)")
    ap.add_argument("--missing-last-sent", choices=["fail","include"], default="fail", help="When time filters are used, include rows with missing LastSentDt")
    ...
    # Parse time filters
    last_before_dt = try_parse_date(args.last_sent_before) if args.last_sent_before else None
    min_days = args.min_days_since_last
    ...
    def process_rows(rows: List[Dict[str,str]], bucket: str):
        for r in rows:
            ...
            if not addr:
                stats[bucket]["missing_addr"] += 1; continue
            if not own:
                stats[bucket]["missing_owner"] += 1; continue
            k = norm_key(addr, own)
            if k in seen_keys:
                stats[bucket]["deduped"] += 1; continue

            if not passes_prior_rules(
                k, tracker, args.prior_exact, args.prior_max, args.min_gap, args.campaign_number,
                min_days, last_before_dt, args.missing_last_sent
            ):
                stats[bucket]["dropped_prior"] += 1; continue

            z5 = get_zip5_from_row(r, addr)
            row = {"PropertyAddress": norm_space(addr), "OwnerName": norm_space(own), "ZIP5": z5, "_src_row": r}
...
# ------------------------- Column detection helpers (kept at bottom to avoid clutter) -------------------------
