...
# -*- coding: utf-8 -*-
"""
Direct Mail - Build Singles + One Combined PDF (clean layout, with Ref footer)
...
- Footer shows only a discreet random reference: Ref: R##### (no batch names or page counts).

Usage (example)
  python direct_mail_batch_and_singles_FIXED.py \
    --csv "ShortTest.csv" \
...
import os, re, csv, argparse, datetime, random
...
def write_single_letter_pdf(out_dir: str, filestub: str, content: str, ref_code: str) -> str:
    ...
    story = build_story(content)  # FIX: Corrected call signature
...
def main():
    ...
    # kept for backward compatibility but not used in footer anymore
    ...
    for i, row in enumerate(rows, start=1):
        ...
        # generate a unique ref code like R12345
