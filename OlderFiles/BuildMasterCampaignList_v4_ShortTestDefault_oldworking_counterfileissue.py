...
Default behavior:
- Emits campaign_master.csv in the EXACT column order of your source CSVs
  (uses the header from the FIRST file in --mandatory; if that fails, tries the
   next mandatory, then optional files; otherwise falls back to a minimal schema).
- No schema/reference file flags are needed.
...
- Prior-history filters against MasterCampaignTracker/MasterPropertyCampaignCounter.csv
...
- Sorted output (ZIP5 → Address → OwnerName) for print order
- Presort reports and postage estimate (rates overridable)
- --debug counters for kept/deduped/dropped/missing
"""
...
def main():
    ...
    for sel in chosen:
        ...
        src = index.get(k)
        # if no direct match (unlikely since candidates came from sources), try from the kept _src_row
        if not src:
            src = sel.get("_src_row", {})
