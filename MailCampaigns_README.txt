MailCampaigns – README (High‑Level Guide + How‑Tos)
Last updated: Aug 23, 2025

1) What this project does (high‑level)
MailCampaigns is a small toolchain that turns raw property/owner CSV lists into print‑ready direct‑mail letters, and maintains a clean history of who you mailed and when. It’s designed to be idempotent (safe to re‑run) and recoverable (you can rebuild the tracker from past campaign folders at any time).
...
Root example: C:\Users\Edit Beluli\Desktop\MailCampaigns
...
Note on mapping file location: the generator writes letters_mapping.csv either directly in the campaign folder or under RefFiles\letters_mapping.csv (both are supported by the finalizer).
...
- Time filters (new):
  ...
  - --missing-last-sent {fail|include} (default fail) → if time filters are set and there’s no LastSentDt, choose to exclude (fail) or allow (include).
...
Template IDs
- Pass --template-id 202 (or any other ID you’ve defined). The mapping and logs record the numeric ID you used.
...
- TemplateIds     → sequence allowing duplicates (e.g., 202|202|303)
...
1) Campaign 1 – build (never mailed):
  cd "C:\Users\Edit Beluli\Desktop\MailCampaigns"
...
2) Generate letters (combined only):
  cd "C:\Users\Edit Beluli\Desktop\MailCampaigns\Campaign_1_Aug2025"
...
5) Generate & finalize:
  cd "C:\Users\Edit Beluli\Desktop\MailCampaigns\Campaign_2_Aug2025"
  python ..\direct_mail_batch_por_POR_KEEP_FIXINDENT.py ^
    --csv "campaign_master.csv" ^
    --outdir "Singles" ^
    --combine-out "letters_batch.pdf" ^
...
Result: In the tracker, shared pairs will show CampaignNumbers = 1|2, CampaignCount = 2, and TemplateIds reflecting the template sequence (e.g., 202|202).

Re‑mail same cohort with different templates
- Build for --prior-exact 2, then 3, etc., and pass a new --template-id each time. The tracker’s TemplateIds stores the sequence (e.g., 202|202|303|404).

Time‑based re‑mailing (e.g., “exactly 4 priors AND last sent ≥ 30 days ago”)
...
Disaster recovery – rebuild tracker & ZIP tally from all campaign folders
  python FinalizeCampaign_TRACKER_STRICT_v5_RECOVER_full_fixdate.py ^
    --rebuild-all ^
    --root "C:\Users\Edit Beluli\Desktop\MailCampaigns" ^
...
- Marker file shows up → That’s expected when using --write-marker. It’s intentionally empty; presence is the signal.
...
- Never delete the campaign folders’ executed_campaign_log.csv if you want to preserve history/rebuild capability.
...
Generate letters (combined only)
  python ..\direct_mail_batch_por_POR_KEEP_FIXINDENT.py ^
    ...
    --map-out "letters_mapping.csv" ^ --template-id 303 --skip-singles ^
...
Rebuild all (disaster recovery)
  python FinalizeCampaign_TRACKER_STRICT_v5_RECOVER_full_fixdate.py ^
    --rebuild-all ^ --root "C:\Users\Edit Beluli\Desktop\MailCampaigns" ^
    --tracker-path "MasterCampaignTracker\MasterPropertyCampaignTracker.csv"
